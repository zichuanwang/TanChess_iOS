...
// -----
	...
	{
		//Note: These two ideas (create header & loading a footer for "not data loaded yet") are disjoint and should not coexist here.  This the case for subclasses
		//of OFTableStaticControllerHelper which want to have a header, yet they don't ever need to "load" data since its setup programatically.  For fear of bugs, I'll
		//leave this how it is now, but, in OFTableStaticControllerHelper you'll see in the viewWillAppear that subclasses are now given a chance to load a header without
		//the "data not loaded" message appearing. [Phil 7/10]
...
- (void)_displayEmptyDataSetView:(UIViewController*)optionalViewController andMessage:(NSString*)message
{
	...
	mEmptyTableController = (OFDeadEndErrorController *)[optionalViewController retain];
...
- (void)_onDataLoadedWrapper:(OFPaginatedSeries*)resources isIncremental:(BOOL)isIncremental
{
	...
	{
		if([self shouldDisplayEmptyDataSet])
		{
...
{	
	...
	if(tableHeaderControllerName)
	{
		if(!mTableHeaderController)
		{
			...
			{
				UIViewController<OFTableControllerHeader>* headerController = (UIViewController<OFTableControllerHeader>*)mTableHeaderController;
...
- (OFResource*)getCellAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    NSUInteger resourceIndex = [self hasStreamingSections] ? dataRow - [self _getLoadedBaseRowIndex] : dataRow;
    return ([section.page.objects count] > resourceIndex) ? [section.page.objects objectAtIndex:resourceIndex] : nil;
}
...
- (OFTableSectionDescription*)getSectionForIndexPath:(NSIndexPath*)indexPath
{
	if (mSections)
	{
		NSAssert2((unsigned int)indexPath.section < [mSections count], @"invalid section index %d in class %s", indexPath.section, class_getName([self class]));
		
		if ((unsigned int)indexPath.section < [mSections count])
		{
...
- (void)toggleExpansionAtIndexPath:(NSIndexPath*)indexPath {
    NSUInteger dataRow, cellType;
    OFTableSectionDescription* section = [self getSectionForIndexPath:indexPath];
    [section gatherDataForRow:indexPath.row dataRow:dataRow cellType:cellType];
    NSAssert(cellType == OFTableSectionCellTypes::Resource, @"Trying to expand a non-resource cell");
    BOOL wasExpanded = [[section.expandedViews objectAtIndex:dataRow] boolValue];
    [section.expandedViews replaceObjectAtIndex:dataRow withObject:[NSNumber numberWithBool:!wasExpanded]];
...
-(BOOL)isExpandedAtIndexPath:(NSIndexPath*)indexPath {
    NSUInteger dataRow, cellType;
    OFTableSectionDescription* section = [self getSectionForIndexPath:indexPath];
    [section gatherDataForRow:indexPath.row dataRow:dataRow cellType:cellType];
    if(cellType == OFTableSectionCellTypes::Resource) {
        return [[section.expandedViews objectAtIndex:dataRow] boolValue];
    }
    else {
        return false;
    }
...
-(NSUInteger)cellTypeAtIndexPath:(NSIndexPath*)indexPath {
    NSUInteger dataRow, cellType;
    OFTableSectionDescription* section = [self getSectionForIndexPath:indexPath];
    [section gatherDataForRow:indexPath.row dataRow:dataRow cellType:cellType];
    return cellType;

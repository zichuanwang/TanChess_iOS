...
- (void)createDeveloperRecommendationsBrowser
{
	...
	controller.title = OFLOCALSTRING(@"More Games");
	[self.navigationController pushViewController:controller animated:YES];
}

- (void)inviteFriends
{
	//Passes nil to get the primary invite definiton
	OFSelectInviteTypeController* controller = [OFSelectInviteTypeController inviteTypeControllerWithInviteIdentifier:nil];
	[self.navigationController pushViewController:controller animated:YES];
}
...
- (NSMutableArray*) buildTableSectionDescriptions
{
	...
	[self createCell:staticCells withTitle:OFLOCALSTRING(@"Add Game as Favorite") andIconName:@"OFFanClubIconStar.png" andControllerName:@"FavoriteThisGame" andCreateSelector:nil];
	if ([OpenFeint localGameProfileInfo].hasiPurchase)
	{
		[self createCell:staticCells withTitle:OFLOCALSTRING(@"Invite a Friend to Play") andIconName:@"OFFanClubIconSendInvite.png" andControllerName:nil andCreateSelector:@selector(inviteFriends)];
	}
	[self createCell:staticCells withTitle:OFLOCALSTRING(@"Developer Announcements") andIconName:@"OFFanClubIconDevAnnouncements.png" andControllerName:@"AnnouncementBrowser" andCreateSelector:nil];
	[self createCell:staticCells withTitle:OFLOCALSTRING(@"Subscribe to Newsletter") andIconName:@"OFFanClubIconNewsLetter.png" andControllerName:@"NewsLetter" andCreateSelector:nil];
	if ([OpenFeint allowUserGeneratedContent])
	{
		[self createCell:staticCells withTitle:OFLOCALSTRING(@"Suggest a Feature") andIconName:@"OFFanClubIconIdea.png" andControllerName:nil andCreateSelector:@selector(createSuggestionBrowser)];
	}
	if ([OpenFeint appHasFeaturedApplication])
	{
		[self createCell:staticCells withTitle:OFLOCALSTRING(@"More Games") andIconName:@"OFFanClubIconMoreGames.png" andControllerName:nil andCreateSelector:@selector(createDeveloperRecommendationsBrowser)];	
	}
...
- (void)onCellWasClicked:(OFResource*)cellResource indexPathInTable:(NSIndexPath*)indexPath
{
	if ([mSections count] > 0)
	{
		...
		if ([staticCells count] > (unsigned int)indexPath.row)
		{
